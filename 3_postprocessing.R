...
classified_r_32636 <- project(classified_r, "epsg:32636")
plot(classified_r_32636, col = palette_raster)
...
classified_r %>% 
  ...
  left_join(habitat_dictionary) %>% 
  ggplot() +
  geom_sf(fill = "transparent", colour = "lightgray", data = snic_ann) +
  geom_raster(aes(x = x, y = y, fill = class_ch)) +
  scale_fill_manual(values = palette_hab) +
  labs(
    x = "", 
    y = "",
    fill = "Тип біотопу"
...
# Save the map
ggsave(
  "./outputs/habitat_map_raster.png",
  width = 26, height = 18, units = "cm", dpi = 300
)
